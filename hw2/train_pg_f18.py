...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        model.add(tf.keras.layers.Dense(size, input_shape=input_placeholder.shape[1:], activation=activation))
...
class Agent(object):
    ...
    def define_placeholders(self):
        ...
        if self.discrete:
            # This is actually a vector of action indices
    ...
    def sum_of_rewards(self, re_n):
        ...
        q_n = []
        for path_ix, path in enumerate(re_n):
            for t in range(len(path)):
                ...
                for t_prime in range(t_start, len(path)):
                    discount = self.gamma ** (t_prime - t_start)
                    trajectory_reward += discount*path[t_prime]
