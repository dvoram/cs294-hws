...
class QLearner(object):
    ...
    def step_env(self):
        ...
        frame_ix = self.replay_buffer.store_frame(self.last_obs)

        rand_step_prob = self.exploration.value(self.t)
        do_random_step = random.random() < rand_step_prob

        if self.model_initialized and not do_random_step:
            encoded_last_obs = self.replay_buffer.encode_recent_observation()
            action_values = self.session.run(self.action_values, feed_dict={
                self.obs_t_ph: np.expand_dims(encoded_last_obs, axis=0)
            })
        ...
        self.replay_buffer.store_effect(frame_ix, action, reward, done)
